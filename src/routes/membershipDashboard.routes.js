...
try {
  ...
  if (waitingSection) {
    ...
    const listUrl = `/ext/members/contact/?action=getListOfMembers&sectionid=${waitingId}&termid=-1&section=${waitingType}&sort=dob`; // Added backticks
    ...
    for (const applicant of waitingList) {
      const individualUrl = `/ext/members/contact/?action=getIndividual&sectionid=${waitingId}&scoutid=${applicant.scoutid}&termid=-1&context=members`; // Added backticks
...
totalWaiting = grouped['Squirrels'].subtotalWaiting + grouped['Beavers'].subtotalWaiting + grouped['Cubs'].subtotalWaiting + grouped['Scouts'].subtotalWaiting;
...
groupName: req.session.groupName || 'OSMHelper.co.uk', // Fallback if not set
totalWaitingFull,
});
}));
